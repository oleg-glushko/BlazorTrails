...
public record GetTrailRequest(int TrailId) : IRequest<GetTrailRequest.Response>
{
    ...
    public record Trail(int Id, string Name, string Location, string? Image, int TimeInMinutes,
        int Length, string Description, string Owner, IEnumerable<Waypoint> Waypoints);
